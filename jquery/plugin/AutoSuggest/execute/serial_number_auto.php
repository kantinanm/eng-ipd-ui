...
/*	if ($len)
	{
		...
		$sql .="   department_asset.serial_number,  ";
		...
		$sql .="  )as mainTb   ";
		$sql .="     ";
		$sql .="  Where  mainTb.serial_number  like '%$q%'  ";
		//$sql .="   AND item.has_child=1 ";
		$sql .="  order by mainTb.serial_number asc limit  $pagesize ";
	 ...
	 while($result = mysql_fetch_array($dbquery)){
		...
		$name =$result['serial_number'] ; // ฟิลที่ต้องการแสดงค่า
